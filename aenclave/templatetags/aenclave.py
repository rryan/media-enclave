...
#======================= SET/UNSET TEMPLATE TAGS =============================#

class SetNode(template.Node):
    """
    A template node that represents the set action.
    """
    ...
    def __init__(self, variable, value):
        ...
        - `variable`: the variable that the set node should set
...
class UnsetNode(template.Node):
    """
    A template node which unsets a variable in the context.
    """
    ...
    def __repr__(self):
        return "<Unset Node: %s>" % self._variable

    def render(self, context):
        del context[self._variable]
        return ''

def do_set(parser, token):
    bits = token.contents.split()
    if len(bits) != 3:
        raise template.TemplateSyntaxError, "%r tag takes two arguments: the name of the variable to set and the value to be set" % bits[0]
    var = bits[1]
    val = bits[2]

    return SetNode(var,val)

def do_unset(parser, token):
    bits = token.contents.split()
    if len(bits) != 2:
        raise template.TemplateSyntaxError, "%r tag takes one argument: the name of the variable to unset" % bits[0]
    var = bits[1]

    return UnsetNode(var)

register.tag('set', do_set)
register.tag('unset', do_unset)
